...
if(isset($_POST['btnsub']) and isset($_POST['users'])){
    ...
    if(!is_dir('../dashboard/users/'.$user.'/EncryptedImages'))
    if(!is_dir('../dashboard/users/'.$user))
    mkdir('../dashboard/users/'.$user);
    else
    mkdir('../dashboard/users/'.$user.'/EncryptedImages');
    $uploadDirectory = '../dashboard/users/'.$user.'/EncryptedImages/';
		    ...
		    <div class="col-75">
                ...
                <option value="">Select image</option>
                    <?php
                        ...
                        $imgs=mysqli_query($connect,"SELECT imageName FROM $name WHERE keyENC='' and typei='upload';");
